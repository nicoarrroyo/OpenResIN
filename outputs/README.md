...
## Current Status
`openresin-label` writes to `labels/` and `patches/` as described above. KRISP-Y will write the model's predictions to `predictions/`, but this is currently being developed. Right now, KRISP-Y writes its predictions to a `.csv` in the satellite image folder which it uses to make its predictions.
