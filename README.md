...
1.  **Navigable Automated Labelling Interface for Regions of Attention (NALIRA)** (`nalira.py`): The `nalira.py` file itself is the orchestrator for the operative file: `nalira_operations.py`. The operative file processes Sentinel-2 satellite imagery, calculates the Normalized Difference Water Index (NDWI), performs cloud masking, and provides an interactive GUI for manually labelling regions of interest (reservoirs, water bodies, land, and sea). It generates the initial labelled coordinates and segments the image data into smaller classified images (mini-chunks) used for training.
...
3.  **KRISP - Predictor (KRISPY)** (`krispy.py`, `krisp.py`): Loads the trained model and deploys it to classify mini-chunks across an entire satellite image tile, generating a prediction file. 
4.  **KRISP External Technical Testing Environment (KRISPETTE)** (`krispette.py`): This component does not run from a clean clone as it stands. This is the evaluation script written for the initial undergraduate project. It is kept in the tree because it is the tool that was used to produce the figures in my dissertation, but it is no longer a tool that works with the rest of the pipeline, and is therefore being rewritten completely. 
...
* **Batch Prediction:** `krispy.py` efficiently processes large images in batches for prediction.
...
**4. Model Deployment/Prediction (krispy)**
* **Configure and Run `krispy.py`:**
    * Open `krispy.py` and modify the `HOME` and `folder` variables.
    * Set the `model_epochs` variable to the number of epochs used for training.
    * Execute the script: `python krispy.py`
    * The script will load the trained model, run predictions, and save the results to a CSV file.

**5. Accuracy Assessment (krispette)**
* **Configure and Run `krispette.py` (! under re-development)**
...
- `src/`: Houses the source code for the project.
    ...
    - `epoch_pathfinder.py`: Helps determine the optimal number of training epochs for `krisp_trainer.py`.
    ...
    - `krispy.py`: Main script for running predictions on full tiles.
    - `krispette.py`: Confusion matrix simulation script. Testing environment for `krisp` models.
    - `krisp_config.py`: TODO
    - `krisp_operations.py`: TODO
	...
	- `nalira_config.py`: TODO
	- `nalira_operations.py`: TODO
